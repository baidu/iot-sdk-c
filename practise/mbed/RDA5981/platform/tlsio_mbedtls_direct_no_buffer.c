...
static void on_underlying_io_open_complete(void* context, IO_OPEN_RESULT open_result)
{
    ...
    else
    {
        ...
        do {
            ...
            printf("start new round of handshake\n");
...
int socket_open(TLS_IO_INSTANCE* tls_io_instance)
{
    ...
    else
    {
        ...
        else
        {
				...
				xxx_dns_gethostbyname(tls_io_instance->hostname,&form.sin_addr.s_addr,ssl_dns_callback,&form.sin_addr.s_addr);
				while(waiting++ < 50)					
				{
				...
				form.sin_port = xxx_htons(tls_io_instance->port);
				
				
				printf("Ip:%s\r\n",xxx_inet_ntoa(&form.sin_addr));
				printf("Port:%d\r\n",tls_io_instance->port);

				xxx_connect(tls_io_instance->socket,(xxx_sockaddr_t*)&form, sizeof(form));

				printf("Block...%d\r\n",errno);

				int flags;

				 if ((-1 == (flags = xxx_fcntl(tls_io_instance->socket, xxx_F_GETFL, 0))) ||
                        (xxx_fcntl(tls_io_instance->socket, xxx_F_SETFL, flags | xxx_O_NONBLOCK) == -1))
                 {
                        printf("Failure: fcntl failure.");
                        xxx_close(tls_io_instance->socket);
                        tls_io_instance->socket = INVALID_SOCKET;
                        result = __FAILURE__;
                 }
