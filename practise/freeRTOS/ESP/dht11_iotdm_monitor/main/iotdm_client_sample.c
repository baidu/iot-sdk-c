...
#define         ADDRESS             "ssl://xxxxxxxxxxxxxxxxxxx.mqtt.iot.gz.baidubce.com:1884"
...
int iotdm_client_run(void)
{
    ...
    TICK_COUNTER_HANDLE msgTickCounter = tickcounter_create();
    tickcounter_ms_t current_time, lastSendTime;

    DHT_Setup(5);  // use GPIO5 of ESP8266
    tickcounter_get_current_ms(msgTickCounter, &current_time);
    ...
    while (iotdm_client_dowork(handle) >= 0)
    {
        ...
        tickcounter_get_current_ms(msgTickCounter, &current_time);
        if (current_time - lastSendTime > 1000 * 5) {
            // sample per 5s
            DHT_Read(&tem, &hum);
            LogInfo("temp: %lf, hum: %lf\n", tem, hum);
            pump->temperature = tem;
            pump->humidity = hum;

            // Sample: use 'serializer' to encode device model to binary and update the device shadow.
            if (CODEFIRST_OK != SERIALIZE(&reported, &reportedSize, pump->temperature, pump->humidity))
            {
            ...
            else
            {
                ...
                memset(reportedString, 0, reportedSize + 1);
